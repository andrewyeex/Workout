...
export class WorkoutProvider extends React.Component {
  ...
  componentDidUpdate = (prevProps, prevState) => {
    if (prevState.activities !== this.state.activities) {
      this.setState(prevState => ({
        activitiesOptions: Object.entries(prevState.activities).map(
          ([key, {name: label}]) => ({label, value: {key, label}, key})
        )
      }))
    }
  }
  ...
  handleError = error =>
    this.setState(
      prevState => ({error: [...prevState.error, error]}),
      console.log({error})
    )
    ...
    try {
      await AsyncStorage.getItem(key, (error, value) => {
        if (!error) {
          ...
          if (key === 'workouts') {
            toAppend.activities = [...activities].map(
              (
                {activitySelected: {key: id}, timeInSeconds: duration},
                order
              ) => ({id, order: order + 1, duration: +duration})
            )
