...
class CreateScreen extends React.Component {
  ...
  handleImagePicker = async () => {
    ...
    if (permission) {
      ...
      if (!result.cancelled)
        this._setStateObj('addWorkoutObj')('image')(result.uri)
    }
  }
  ...
  handleCameraPermission = async () => {
    ...
    if (status !== 'granted') {
      const {status: _status} = await Permissions.askAsync(
  ...
  handleAddActivities = activity => () =>
    this.setState(prevState => ({
      ...
      addWorkoutObj: {
        ...
        duration: [...prevState.addWorkoutObj.activities, activity].reduce(
          (acc, curr) => +acc + +curr['timeInSeconds'],
          0
        )
      }
    }))

  handleRemoveActivities = index => () =>
    this.setState(prevState => ({
      addWorkoutObj: {
        ...prevState.addWorkoutObj,
        activities: [...prevState.addWorkoutObj.activities].filter(
          (v, i) => i !== index
        )
      }
    }))
  ...
  render() {
    ...
    return mode === 0 ? (
      <CreateMenu _setState={_setState} />
    ) : (
      <CreateAdd {...createAddProps} />
    )
...
const CreateMenu = ({_setState}) => (
  <View style={{flex: 1}}>
    <TextHeader
      text='ADD'
      callback={_setState('mode')(1)}
      fontStyle={{color: '#fff'}}
      style={{justifyContent: 'center', backgroundColor: '#000'}}
    />
    <TextHeader
      text='REMOVE'
      callback={_setState('mode')(2)}
      style={{justifyContent: 'center'}}
    />
    <TextHeader
      text='EDIT'
      callback={_setState('mode')(3)}
      fontStyle={{color: '#fff'}}
      style={{justifyContent: 'center', backgroundColor: '#000'}}
    />
  </View>
